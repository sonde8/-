...
const Home = () => {
  ...
  const getMovieList = async ()=>{
    ...
    // API 요청을 하기 위해서 Promise 미리 생성
    const popularList = api.get('/movie/popular?language=ko&page=1')
    const topRatedList = api.get('/movie/top_rated?language=ko&page=1') 
    const upComingList = api.get('/movie/upcoming?language=ko&page=1')
    const genreList = api.get('/genre/movie/list?language=ko')

    // 모든 API 요청을 병렬 형태로 실행, 결과 값을 배열로 받음
    // 배열의 비구조할당을 이용해 각각의 값을 변수로 저장
    ...
    // 데이터 요청이 완료되면 loading 상태를 false로 변경
    setLoading(false)
